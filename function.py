...
class tfidf_centroid():
    ...
    def __init__(self):
        self.vectorizer = TfidfVectorizer(max_df = 1.,sublinear_tf = True)
    ...
    def fit(self, df_split_word_train, mid_send_recip_train):
        ...
        for index, row in address_book_train.iterrows():
            ...
            if row[0] in self.dict_centroid.keys():
                list_idx = df_split_word_train[df_split_word_train['mid'].isin([i for i in row[2]])].index.tolist()
                if list_idx != []:
    ...
    def predict(self, test_set, df_split_word_test, number_keep, sender):       
        corpus_test = df_split_word_test['word split'].tolist() 
        self.X_test = self.vectorizer.transform(corpus_test)
        ...
        for idx in range(self.X_test.shape[0]):
            ...
            prob_list = [] 
            tot_cosine = 0
            for recip in self.dict_centroid[sender].keys():
                cosine = linear_kernel(self.X_test[idx], self.dict_centroid[sender][recip])/norm(scipy.sparse.csr_matrix(self.dict_centroid[sender][recip]))
                tot_cosine += cosine
                ...
                self.cosine_list.append((recip,float(cosine)))
            ...
            self.final_prob = sorted(self.final_prob, key=lambda prob: prob[1], reverse=True)[:number_keep]
            self.final_prob = [x for x in self.final_prob if x[1]>0.]
            
            predict_test.append((df_split_word_test['mid'][idx], ' '.join([x[0] for x in self.final_prob])))
